...
<div class="container my-5">
    @foreach ($posts as $post)
        <div class="card mb-5">
            <div class="row g-0">
                <div class="col-md-5">
                    <img src="img/{{ $post["image"] }}" class="img-fluid rounded-start" alt="{{ $post["slug"] }}">
                </div>
                <div class="col-md-7">
                    <div class="card-body">
                        <h4 class="card-title fw-bolder">{{ $post["title2"] }}</h4>
                        <p class="card-text"><small class="text-body-secondary text-muted text-uppercase"><strong>{{ $post["date"] }}</strong></small></p>
                        <p class="card-text">{{ $post["bodysmall"] }}</p>
                        <p>
                            <a href="/posts/{{ $post["slug"] }}">Lihat lebih lengkap</a>
                        </p>
                    </div>
                </div>
            </div>
